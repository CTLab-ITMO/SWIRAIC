...
# ЗАМОРОЗИМ РАННИЕ ENCODER LAYERS (enc1, enc2, enc3)
...
# РАЗМОРОЗИМ ГЛУБОКИЕ ENCODER LAYERS (enc5) + BOTTLENECK
for param in model.enc5.parameters():
    param.requires_grad = True
for param in model.bottleneck_conv.parameters():
    param.requires_grad = True

# РАЗМОРОЗИМ ВСЕ DECODER СЛОИ
...
print("  enc1: FROZEN (базовые текстуры)")
print("  enc2: FROZEN (низкоуровневые признаки)")
print("  enc3: FROZEN (средние признаки)")
print("  enc4: TRAINABLE (глубокие признаки)")
print("  enc5: TRAINABLE (самые глубокие признаки)")
print("  bottleneck: TRAINABLE")
print("  decoder: TRAINABLE (полностью адаптируется)")

total_params_after = sum(p.numel() for p in model.parameters())
trainable_params_after = sum(p.numel() for p in model.parameters() if p.requires_grad)
frozen_params = total_params_after - trainable_params_after

print(f"  Всего параметров: {total_params_after:,}")
print(f"  Обучаемых: {trainable_params_after:,}")
print(f"  Замороженных: {frozen_params:,}")
print(f"  Процент обучаемых: {trainable_params_after/total_params_after*100:.1f}%")
print(f"  Процент замороженных: {frozen_params/total_params_after*100:.1f}%")
...
class CompositeLoss(nn.Module):
    def __init__(self, criterion_bce, lambda1=2.0, lambda2=1.5,
                 alpha_t=0.6, beta_t=0.4, gamma_t=1.5,
                 gamma_f=2.5, alpha_f=0.5):
        super().__init__()
    ...
    def tversky_loss(self, y_true, y_pred):
        ...
        return (true_pos + smooth) / (true_pos + self.alpha_t * false_neg + self.beta_t * false_pos + smooth)

    def focal_loss(self, y_true, y_pred):
        y_true = y_true.view(-1)
        y_pred = torch.sigmoid(y_pred).view(-1)
        ce_loss = F.binary_cross_entropy(torch.sigmoid(y_pred), y_true, reduction='none')
        modulating_factor = torch.pow(1 - torch.sigmoid(y_pred), self.gamma_f)
        loss = self.alpha_f * modulating_factor * ce_loss
        return loss.mean()

    def forward(self, y_pred, y_true):
        bce = self.criterion_bce(y_pred, y_true)
        focal = self.focal_loss(y_true, y_pred)
        tversky = self.tversky_loss(y_true, y_pred)

        # BCE*(1+λ1*focal) + λ2*(1-tversky)^γt
        loss = bce * (1 + self.lambda1 * focal) + self.lambda2 * torch.pow((1 - tversky), self.gamma_t)
        return loss

criterion = CompositeLoss(
    criterion_bce=criterion_bce,
    lambda1=2.0,      # Усиление BCE через focal
    lambda2=1.5,      # Вес Tversky
    alpha_t=0.6, beta_t=0.4, gamma_t=1.5,  # Tversky параметры
    gamma_f=2.5, alpha_f=0.5               # Focal параметры
)
...
for epoch in range(NUM_EPOCHS):
    ...
    if avg_iou > best_iou:
        best_iou = avg_iou
        best_epoch = epoch + 1
        checkpoint_path = os.path.join(CHECKPOINT_DIR, 'Stage2-U-Net_RN50_Imitation_SWIR_Dataset_best.pth')
...
final_checkpoint_path = os.path.join(CHECKPOINT_DIR, 'Stage2-U-Net_RN50_Imitation_SWIR_Dataset_final.pth')
...
print(f" Время обучения: {training_time}")
print(f" Лучшая модель: {os.path.join(CHECKPOINT_DIR, 'Stage2-U-Net_RN50_Imitation_SWIR_Dataset_best.pth')}")
print(f" Финальная модель: {final_checkpoint_path}")

import shutil
shutil.make_archive('/kaggle/working/checkpoints_u-net-rn50-stage2-infraloss', 'zip', '/kaggle/working/checkpoints')

#test_images_path = "/kaggle/input/test-stage-2/test stage 2/images/test"
#test_masks_path = "/kaggle/input/test-stage-2/test stage 2/annotations/test"
...
os.makedirs('Stage2-U-Net-rn50-predictions-real-infra', exist_ok=True)
...
for idx in tqdm(range(num_viz), desc="Saving visualizations", total=num_viz):
    ...
    save_path = f'Stage2-U-Net-rn50-predictions-real-infra/{save_name}'
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()

import shutil
shutil.make_archive('Stage2-U-Net-rn50-predictions-real-infra', 'zip', 'Stage2-U-Net-rn50-predictions-real-infra')